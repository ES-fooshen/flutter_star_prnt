    ...
    private fun appendCommands(
        ...
        printCommands?.forEach {
            if (it.containsKey("appendCharacterSpace"))
                ...
                else builder.appendAbsolutePosition(
                    (it.get("appendAbsolutePosition").toString()).toInt()
                )
                ...
                builder.appendHorizontalTabPosition(
                    it.get("appendHorizontalTabPosition") as IntArray
                ) // TODO: test this in the future
                ...
                val barcodeWidth: ICommandBuilder.BarcodeWidth =
                    if (it.containsKey("BarcodeWidth")) getBarcodeWidth(
                        it.get("BarcodeWidth").toString()
                    )
                ...
                val diffusion: Boolean =
                    if (it.containsKey("diffusion")) (it.get("diffusion")
                        .toString()).toBoolean() else true
                val width: Int =
                    if (it.containsKey("width")) (it.get("width").toString()).toInt() else 576
                val bothScale: Boolean =
                    if (it.containsKey("bothScale")) (it.get("bothScale")
                        .toString()).toBoolean() else true
                ...
                val diffusion: Boolean = if (it.containsKey("diffusion")) (it.get("diffusion")
                    .toString()).toBoolean() else true
                val width: Int =
                    if (it.containsKey("width")) (it.get("width").toString()).toInt() else 576
                val bothScale: Boolean = if (it.containsKey("bothScale")) (it.get("bothScale")
                    .toString()).toBoolean() else true
                val rotation: ICommandBuilder.BitmapConverterRotation =
                    if (it.containsKey("rotation")) getConverterRotation(
                        it.get("rotation").toString()
                    ) else getConverterRotation("Normal")
                try {
